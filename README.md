...
**一段文案，出全套小红书图文** —— 四步工作流：长文 → 文字卡片 → AI 封面 → 导出发布

![总览](docs/screenshots/overview.png)

**一段文字，出小红书调性长文** —— 先抽结构，再生成长文，还能顺手优化表达

![长文](docs/screenshots/article.png)

**四种高级感卡片风格任选** —— 同一段内容，一键切换不同审美表达

![卡片风格](docs/screenshots/card-styles.png)

**一键排版，直出高级感卡片** —— 边写边预览，边改边调，卡片效果实时可见

![排版](docs/screenshots/cards.png)

**图片打包好，发布文案也写好** —— 下载 ZIP，复制文案，整理完就能直接发

![导出](docs/screenshots/export.png)

> AI 封面（AI Cover）的演示图稍后补上。
...
打开浏览器访问终端里给出的本地地址，在设置里填入你的中转地址和 API Key，就能开始。
...
本项目以 **GNU AGPL-3.0** 开源：你可以自由使用、修改、分发，但**任何修改版（包括作为网络服务运行的版本）都必须同样以 AGPL-3.0 公开源码**。详见 [LICENSE](LICENSE)。

Copyright (C) 2026 Liberty233333
